...
#           you may do so by referencing www.superdatascience.com in your work.
...
# *** You can run the entire script by doing a ctrl + A and Ctrl + Enter
...
typeof(Points)
# Note that matrix keyword has not been used to create the matrix. 
# Insead the rbind function discussed in previous lecture that 
# is used to bind rows together has been used.
...
# Use the Games matrix to get games from 2010 to 2015 for top 3 players

Games[6:10]

Games[Rows, Cols]

is.vector(Games[2, 6:10])

is.matrix(Games[1:2, 6:10])


vector1<- c(1:10)
vector1

# Get me the first and the tenth row from Games
# Hint - Use combine

Games[c(1,10),]


# Using the names of the columns as input get me the data for 
# 2008 and 2009 seasons for all player
...
is.matrix(Games[1:3, 6:10])

# Using drop parameter in subsetting
Games[1,]
