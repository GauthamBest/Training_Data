...
# all records with Ozone not null
filter(.data = airquality, !is.na(airquality$Ozone))

# Records that do not have any columns with NULL
airquality[complete.cases(airquality),]

filter(.data = airquality, complete.cases(airquality))
...
sqldf('select * from airquality order by Temp desc limit 10')
